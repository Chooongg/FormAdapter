...
import com.chooongg.form.FormOptionLoader
import com.chooongg.form.FormUtils
import com.chooongg.form.FormViewHolder
import com.chooongg.form.enum.FormOptionLoadMode
...
abstract class BaseOptionForm<T>(name: Any?, field: String?) : BaseForm(name, field) {
    ...
    fun clearOptions() {
        if (!localOptions.isNullOrEmpty()) clearLocalOptions()
        clearLoadedOptions()
    }

    fun clearLocalOptions() {
        localOptions = null
    }

    fun clearLoadedOptions() {
        optionLoadResult = OptionLoadResult.Wait()
    }
